---
layout: page
title: "Math 013 Syllabus (Fall 2026)"
permalink: /teaching/2026/math130fall/013syllabus/
parentlink: /teaching/2026/math130fall/
---

Class Information
=====

**Course Title** Math 013: Support Topics for Quantitative Reasoning

**Instructor** Julia Williams

**Website** [http://juliakw.net/teaching/2026/math130fall/]({{ site.baseurl }}/teaching/2026/math130fall/)

**Email** `jwilliams@capecod.edu`

**Phone** (774) 330-4260

**Primary out of class contact** course website or email me

**Class Hours and Room** TODO

**Office Hours** TBA

**Textbook** *None*

**Course Description** This course is designed to pair with a college-level Quantitative Reasoning class. It provides a review of
the prerequisite mathematical skills needed in Quantitative Reasoning. Students receive targeted support with
arithmetic and algebra topics using a “just-in-time” approach. This course also reinforces topics covered in the paired
Quantitative Reasoning course

**Prerequisite** MAT011 or satisfactory basic skills assessment score

**Departmental Syllabus** (with formal learning outcomes) [https://www.capecod.edu/media/capecodedu/content-assets/documents/syllabi/MAT013-Support-Topics-for-Quantitative-Reasoning-2024.pdf](https://www.capecod.edu/media/capecodedu/content-assets/documents/syllabi/MAT013-Support-Topics-for-Quantitative-Reasoning-2024.pdf)

Informal Learning Outcomes
========

This course is a companion to Math 130 designed as a support to provide you with extra practice for the foundations of that course.


Grading Policy
=======

This course is graded on a Pass/Retake/Fail system. Here are the specifications you are required to meet for each outcome.

* **Pass.** Complete all but three in-class activities, or else acheive a C or better in the companion Math 130 class.

* **Retake.** Not meet the requirements for a Pass, but remain active in this class  throughout the semester.

* **Fail.** Not meet the requriements for a Retake, namely stop participating in this class and the companion Math 130 class. 
	
In-Class Exercises
========

This class will be used for extra practice to support the companion Math 130 class. We meet immediately before Math 130, and will use the time to refresh you on math topics needed for that class.

This extra pratice will take the form of writing and thinking exercises. You will write in accordance to a prompt, share what you wrote with classmates, and respond to their ideas. As such, every day you need to bring a notebook and writing utensil. You will occasionally need to tear out a page from your notebook.

To enable the goals of this class, we are as low tech as possible. I ask that you refrain from using laptops, phones, and other electronic devices during class time. (The exception, of course, is if you need a piece of technology for an accessibility accommodation.)

These exercises are graded based on participation. To earn credit for this support class you must show up and you must actively participate.

Textbook Information
========

There is no textbook associated to this class.

Attendance and Class Participation Guidelines
==========

You are expected to attend class sessions, as the material for this class is entirely contained within the scheduled class time. 

Mathematics has a reputation for being removed from social concerns and identities. Whether or not this is true for the content of mathematics, it is certainly false  for the process of learning mathematics. Our classroom is to be a welcoming one, where everyone feels able to participate and learn regardless of their background or identity. As learners it is your obligation to treat others with respect and generosity, and be willing to exchange ideas with others.


Communication Policy and Office Hours
=============

Announcements and homework will be posted to the course website. 

The best way to contact me outside of class is by email. Please put "math 013" in the subject line of your email. If I do not respond by the end of the next weekday, please send me a follow up reminder message.

Office hours are held multiple times in the week, to give you an opportunity to ask questions and receive help in-person outside of class time. If you prefer to meet at a different time, please contact me to arrange that.


Accessibility
=============

Students with disabilities are legally entitled to reasonable accommodations to ensure equal access to education. I am committed to providing you with equal access to this class, and am happy to work with you to ensure reasonable accommodations. Because the accommodations offered are usually forward-looking modifications, it is important to get them set up as soon as possible.

Anyone who feels they may need accommodation based on the impact of a disability should contact the [O'Neill Center for Student Access and Support](https://capecod.edu/oneillcenter/) (774-330-4453; `oneillcenter@capecod.edu`)

The Americans with Disabilities Act defines a disability as a medical condition that substantially limits one or more major life activities—including things like walking, sleeping, taking care of yourself, learning, and regulating your emotions—or major bodily functions. If you have a medical condition—including mental health conditions—that significantly interferes with your schoolwork, you probably qualify. You do not need to disclose your condition to your instructors to receive accommodations. 

Academic Honesty and AI Policy
========

You are expected to know and uphold the college’s policies on academic honesty as described in the Student Handbook. Mathematics classes form part of the core base of skills you need to succeed in many later classes, and you are harming yourself if you try to avoid learning the material for this class.

Calendar
========

The schedule for this class runs in parallel to the companion Math 130 class.

* Unit 1 (9/8–10/8): Logic

  * Week 1: Intro, What is math?

  * Week 2: Notetaking, What is counting?

  * Week 3: Sets and their operations, Math anxiety

  * Week 4: Reading math, Sets and subsets

  * Week 5: Notecard, Pre-exam refreshing

* Unit 2 (10/13–11/12): Statistics

  * Week 6: Numbers in the news, Graphs

  * Week 7: Fractions, Percents

  * Week 8: Summarizing data, Radicals

  * Week 9: Probability, Arguments

  * Week 10: Notecard, Pre-exam refreshing

* Unit 3 (11/17–12/10): Modeling

  * Week 11: Covariation, Cartesian coordinates

  * Week 12: Lines, Exponents

  * Week 13: Logarithms, Limitations of models

  * Week 14: Notecard, Pre-exam refreshing


Notice of Changes
======

This syllabus is subject to change. If this happens, you will be informed.


